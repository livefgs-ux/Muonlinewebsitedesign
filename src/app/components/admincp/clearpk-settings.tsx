...
import { UserX, CheckCircle, XCircle, Save, Skull } from 'lucide-react';
import { Card } from '../ui/card';
import { Button } from '../ui/button';

interface ClearPKConfig {
  active: number;
  zen_cost: number;
...
export function ClearPKSettings() {
  // Mock current configuration - would come from XML file in production
  const [config, setConfig] = useState<ClearPKConfig>({
    active: 1,
    zen_cost: 5000000,
    credit_config: 1,
    credit_cost: 100,
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      console.log('💾 Saving Clear PK Settings...');
      console.log('📄 XML Path: module_configs/usercp.clearpk.xml');
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="text-3xl text-white mb-2">Clear PK Settings</h2>
      </div>
                ...
                <th className="px-6 py-4 text-left align-top bg-black/20" style={{ width: '40%' }}>
                  <div>
                    ...
                    <div className="text-gray-400 text-sm font-normal">
                      Enable/disable the clear pk module.
                    </div>
                  </div>
                </th>
              ...
              <tr className="border-b border-gray-700/30 hover:bg-white/5 transition-colors">
                <th className="px-6 py-4 text-left align-top bg-black/20">
                  <div>
                    ...
                    <div className="text-gray-400 text-sm font-normal">
                      Amount of zen required to clear the character pk status. Set to 0 to disable zen requirement.
                    </div>
                  </div>
                </th>
                <td className="px-6 py-4">
              ...
              <tr className="border-b border-gray-700/30 hover:bg-white/5 transition-colors">
                <th className="px-6 py-4 text-left align-top bg-black/20">
                  <div>
                    ...
                    <div className="text-gray-400 text-sm font-normal">
                      Amount of credits required to clear the character pk status. Set to 0 to disable credit requirement.
                    </div>
                  </div>
                </th>
                <td className="px-6 py-4">
      ...
      <Card className="backdrop-blur-lg bg-blue-500/5 border-blue-500/30 p-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="text-white font-semibold mb-2">Clear PK Settings Information</h3>
            <p className="text-blue-300/70 text-sm mb-2">
              Configure the Clear PK module that allows players to clear their PK (Player Killer) status via the UserCP.
            </p>
            <ul className="text-blue-300/70 text-sm space-y-1 list-disc list-inside">
              ...
              <li><strong>Zen Cost:</strong> Amount of in-game zen required to clear PK status (0 = free)</li>
              <li><strong>Credit Cost:</strong> Amount of credits required to clear PK status (0 = free)</li>
              <li><strong>Credit Configuration:</strong> Which credit type to use (WCoin, GoblinPoint, etc.)</li>
              <li>Settings are saved to: <code className="bg-black/50 px-2 py-1 rounded text-xs">module_configs/usercp.clearpk.xml</code></li>
              <li>PK status makes characters appear red and can affect gameplay mechanics</li>
              <li>This module allows players to remove their PK status for a fee</li>
            </ul>
          </div>
        </div>
      </Card>
