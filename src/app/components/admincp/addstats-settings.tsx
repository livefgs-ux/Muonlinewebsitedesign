...
import { Settings, CheckCircle, XCircle, Save, TrendingUp } from 'lucide-react';
import { Card } from '../ui/card';
import { Button } from '../ui/button';

interface AddStatsConfig {
  active: number;
  zen_cost: number;
...
export function AddStatsSettings() {
  // Mock current configuration - would come from XML file in production
  const [config, setConfig] = useState<AddStatsConfig>({
    active: 1,
    zen_cost: 1000000,
    credit_config: 1,
    credit_cost: 50,
    required_level: 400,
    required_master_level: 0,
    max_stats: 32767,
    minimum_limit: 1,
  });
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // Validate setting_7 (max_stats)
      if (!settings.setting_7) throw new Error('Invalid setting (max_stats)');
      if (!validateUnsignedNumber(settings.setting_7)) {
        throw new Error('Invalid setting (max_stats)');
      }

      // Validate setting_8 (minimum_limit)
      if (!settings.setting_8) throw new Error('Invalid setting (minimum_limit)');
      if (!validateUnsignedNumber(settings.setting_8)) {
        throw new Error('Invalid setting (minimum_limit)');
      }

      // All validations passed - save to XML
      console.log('💾 Saving Add Stats Settings...');
      console.log('📄 XML Path: module_configs/usercp.addstats.xml');
      console.log('⚙️ Settings:', {
        ...
        max_stats: Number(settings.setting_7),
        minimum_limit: Number(settings.setting_8),
      });
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div>
        <h2 className="text-3xl text-white mb-2">Add Stats Settings</h2>
      </div>
          ...
          <table className="w-full">
            <tbody>
                ...
                <th className="px-6 py-4 text-left align-top bg-black/20" style={{ width: '40%' }}>
                  <div>
                    ...
                    <div className="text-gray-400 text-sm font-normal">
                      Enable/disable the add stats module.
                    </div>
                  </div>
                </th>
              ...
              <tr className="border-b border-gray-700/30 hover:bg-white/5 transition-colors">
                <th className="px-6 py-4 text-left align-top bg-black/20">
                  <div>
                    ...
                    <div className="text-gray-400 text-sm font-normal">
                      Amount of zen required to add stats. Set to 0 to disable zen requirement.
                    </div>
                  </div>
                </th>
                <td className="px-6 py-4">
              ...
              <tr className="border-b border-gray-700/30 hover:bg-white/5 transition-colors">
                <th className="px-6 py-4 text-left align-top bg-black/20">
                  <div>
                    ...
                    <div className="text-gray-400 text-sm font-normal">
                      Amount of credits required to add stats. Set to 0 to disable credit requirement.
                    </div>
                  </div>
                </th>
                <td className="px-6 py-4">
              ...
              <tr className="border-b border-gray-700/30 hover:bg-white/5 transition-colors">
                <th className="px-6 py-4 text-left align-top bg-black/20">
                  <div>
                    ...
                    <div className="text-gray-400 text-sm font-normal">
                      Minimum level required to add stats. Set to 0 to disable level requirement.
                    </div>
                  </div>
                </th>
                <td className="px-6 py-4">
              ...
              <tr className="border-b border-gray-700/30 hover:bg-white/5 transition-colors">
                <th className="px-6 py-4 text-left align-top bg-black/20">
                  <div>
                    ...
                    <div className="text-gray-400 text-sm font-normal">
                      Minimum master level required to add stats. Set to 0 to disable master level requirement.
                    </div>
                  </div>
                </th>
                <td className="px-6 py-4">
              ...
              <tr className="border-b border-gray-700/30 hover:bg-white/5 transition-colors">
                <th className="px-6 py-4 text-left align-top bg-black/20">
                  <div>
                    <div className="text-white font-medium mb-1">Maximum Stats</div>
                    <div className="text-gray-400 text-sm font-normal">
                      Amount of points that each stat may have.
                    </div>
                  </div>
                </th>
                <td className="px-6 py-4">
                  <input
                    type="text"
                    name="setting_7"
                    defaultValue={config.max_stats}
                    className="w-full px-4 py-2 bg-black/40 border border-gray-700 rounded-lg text-white focus:border-sky-500 focus:outline-none transition-colors"
                  />
                </td>
              </tr>

              {/* Minimum Points Limit */}
              <tr className="border-b border-gray-700/30 hover:bg-white/5 transition-colors">
                <th className="px-6 py-4 text-left align-top bg-black/20">
                  <div>
                    <div className="text-white font-medium mb-1">Minimum Points Limit</div>
                    <div className="text-gray-400 text-sm font-normal">
                      Minimum amount of points the player must add in order to use the module.
                    </div>
                  </div>
                </th>
                <td className="px-6 py-4">
                  <input
                    type="text"
                    name="setting_8"
                    defaultValue={config.minimum_limit}
                    className="w-full px-4 py-2 bg-black/40 border border-gray-700 rounded-lg text-white focus:border-sky-500 focus:outline-none transition-colors"
                  />
                </td>
              </tr>
      ...
      <Card className="backdrop-blur-lg bg-blue-500/5 border-blue-500/30 p-6">
        <div className="flex items-start gap-3">
          ...
          <div>
            <h3 className="text-white font-semibold mb-2">Add Stats Settings Information</h3>
            <p className="text-blue-300/70 text-sm mb-2">
              Configure the Add Stats module that allows players to add stat points via the UserCP.
            </p>
            <ul className="text-blue-300/70 text-sm space-y-1 list-disc list-inside">
              ...
              <li><strong>Zen Cost:</strong> Amount of in-game zen required per stat point (0 = free)</li>
              <li><strong>Credit Cost:</strong> Amount of credits required per stat point (0 = free)</li>
              <li><strong>Credit Configuration:</strong> Which credit type to use (WCoin, GoblinPoint, etc.)</li>
              <li><strong>Required Level:</strong> Minimum character level needed to use the module (max 400)</li>
              <li><strong>Required Master Level:</strong> Minimum master level needed to use the module</li>
              <li><strong>Maximum Stats:</strong> Maximum value for each individual stat (Strength, Agility, etc.)</li>
              <li><strong>Minimum Points Limit:</strong> Minimum number of points player must add at once</li>
              <li>Settings are saved to: <code className="bg-black/50 px-2 py-1 rounded text-xs">module_configs/usercp.addstats.xml</code></li>
            </ul>
          </div>
        </div>
      </Card>
