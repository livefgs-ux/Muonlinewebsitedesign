<?php
/**
 * Ranking de Master Level
 * Gera JSON com top 100 personagens por master level
 */
...
try {
    ...
    $sql = "SELECT
                c.Name as name,
                c.cLevel as level,
                IFNULL(c.MasterLevel, 0) as masterLevel,
                c.Resets as resets,
                c.Class as classId,
                CASE c.Class
            ...
            FROM Character c
            WHERE c.CtlCode = 0
            ORDER BY masterLevel DESC, c.cLevel DESC
            LIMIT 100";
    ...
    foreach ($results as $index => $row) {
        $ranking[] = [
            ...
            'name' => $row['name'],
            'class' => $row['class'],
            'masterLevel' => (int)$row['masterLevel'],
            'level' => (int)$row['level'],
    ...
    file_put_contents(__DIR__ . '/data/masterlevel_ranking.json', json_encode($json_data, JSON_PRETTY_PRINT));
    
    echo "Master Level ranking updated: " . count($ranking) . " players\n";
